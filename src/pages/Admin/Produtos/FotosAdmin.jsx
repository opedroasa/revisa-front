  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-2xl font-semibold">Fotos — {produto?.nome}</h2>
        <div className="flex items-center gap-2">
          <input
            type="file"
            accept="image/*"
            onChange={(e)=>setFile(e.target.files?.[0] || null)}
            className="block w-64 text-sm file:mr-3 file:rounded-md file:border-0 file:bg-brandNavy file:px-3 file:py-2 file:text-white"
          />
          <Button onClick={enviar} disabled={!file}>Upload</Button>
      ...
      {fotos.length === 0 && <p className="text-gray-500">Nenhuma foto enviada.</p>}

      <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
        {fotos.map(f => (
          <div key={f.id} className={"rounded-xl border p-2 " + (f.destaque ? "ring-2 ring-brandNavy" : "")}>
            ...
            <div className="mt-2 flex items-center justify-between gap-2">
              ...
              <Button className="bg-red-600 hover:bg-red-700" onClick={()=>excluir(f.id)}>
                Excluir
              </Button>
            </div>
          </div>
